...
if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
	...
	if ($debug) {
		echo "QUERY: SELECT passwd FROM users WHERE email = '$user'";
	}
	
	$res = pg_query ( $pgh, "SELECT passwd,fname,role FROM users WHERE email = '$user'" );
	if (! $res) {
		if ($debug) {
			echo " :: ", pg_last_error ( $pgh ), "<p>";
		}
	}
	...
	if (password_verify ( "$passwd", "$userRecord[0]" )) {
		...
		if ($debug) {
			echo "<h2>Password check for $user failed</h2>";
		}
...
<div class='register_box'>
	<div id="login-unauth" class='mcontents_box'>
		...
		<h1>Login</h1>
		<p>Enter a username and password. If first time, click 'Create
			Account'.</p>
		<form action="index.php" method="post" class="user_login">
			<ul>
				<li><input class="login" name="user" type="email"
					placeholder="you@domain.com" required></li>
				<li><input class="login" name="passwd" type="password"
...
<div class="login-auth">
	...
	<div class="account_text">
		<span class="account_header">User:</span> <?php echo $_SESSION['user']; ?><p>
	...
	<div class="account_text">
		<span class="account_header">Name:</span> <?php echo $_SESSION['name']; ?><p>
	...
	<div class="account_text">
		<span class="account_header">Role:</span> <?php echo $_SESSION['role']; ?><p>
